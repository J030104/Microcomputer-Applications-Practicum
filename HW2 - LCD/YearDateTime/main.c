...
int main()
{
	...
	// Initial RTC to 2023/04/14 Thu. 15:30:00
	InitialLCD();	//Initial LCD Module
	InitialRTC(year, month, week, day, hour, min, sec); //Initial RTC Module
		...
		if (key != 0xFF){
			if(key == 14){		//SW16 - Clear to 00
				...
				if(setsec == 2){
					...
					InitialRTC(year, month, week, day, hour, min, nddigit); //Initial RTC Module
				}
				else if(setsec == 0){
					InitialRTC(year, month, week, day, hour, min, final); //Initial RTC Module
				}
...
void SendandSet(int secnum){
	if(setsec == 2){			//Setting second digit
		if(secnum >= 6){
			return;
		}
		nddigit = secnum << 4;
	...
	else if(setsec == 1){		//Setting first digit
		final = nddigit + secnum;
		InitialRTC(year, month, week, day, hour, min, final); //Initial RTC Module
		DisplayTime();
		setsec--;
	}
